...
export class FileSystemStorageProvider {
    ...
    async createProject(projectId, projectName, initialFiles = {}) {
        ...
        const readmeFilename = `00_README_${projectName}.txt`;
        
        try {
            const fileHandle = await directoryHandle.getFileHandle(filename, { create: true });
            const writable = await fileHandle.createWritable();
            
            // Prepare file metadata (without content for project JSON)
            const fileMetadata = {};
            for (const [fileId, fileData] of Object.entries(initialFiles)) {
                // Create separate .mmd file for each diagram
                ...
                // Store only metadata in project JSON
                fileMetadata[fileId] = {
                    id: fileData.id,
                    name: fileData.name,
                    type: fileData.type,
            ...
            const readmeContent = `To open this project, go to ${window.location.href}, select Project > Open folder..., then select this folder.`;
